...
| `barra-tarefas.c` | C + Xlib cru, 2,67 MB de RSS (392 kB de PSS). Fonte **core**, iso8859-1. **Escura** desde 03/08/2026 (`#define BARRA_ESCURA`; a paleta clara amostrada do `Untitled.png` continua no código, atrás do `#else`), relógio em `fixed-bold-13`. Desde 07/08/2026 tem **só relógio e desligar**, em 630 linhas contra 1484: volume, áudio/câmera, Coisas, atalhos de app e `[+]` mudaram para a **doca do panorama**. Com isso ela parou de perguntar — eram 4 `pactl` a cada 2 s e um `transferir-usb estado` a cada 30 s, para sempre; agora o único motivo de acordar é o minuto virar |
...
| `panorama.c` | **aperta Win e vê tudo**: grade das janelas abertas, cada uma com a miniatura viva, em 0,73 MB de PSS enquanto nunca foi aberto. Daemon, sobe no `startwm.sh`. Detecta a tecla Win sozinha com **XInput2 cru** (sem grab, senão roubaria o Super do xfwm4); a miniatura vem do **Composite** redirecionado, escalada pelo **XRender** dentro do servidor. Desde 03/08/2026. Desde 07/08/2026 carrega também a **doca** no rodapé do painel — o que era a barra de tarefas: `Vol`/`Mic` com mudo, calha e menu de dispositivos, `Audio`/`Camera`, `Coisas`, os atalhos de app em 32 px e o `[+]`. Depois da primeira abertura fica em 2,78 MB de PSS (fontconfig + os Pixmaps dos ícones), e cada abertura custa ~38 kB |
...
| `barra-apps` | atalhos de aplicativo da **doca** (o nome é de onde ele nasceu): lê `.desktop`, converte o ícone e mantém o `apps.conf`. Programa nosso só entra na lista do `[+]` se tiver um `.desktop` em `desktop/`. O ícone é **32 px** desde 07/08/2026, e o lado entra no nome do cache junto com a cor |
...
```
panorama.c (a doca)  ──exec──>  transferir-usb  ──exec──>  audio-padrao.ps1 (interop)
       │                              │
       ├────exec────>  camera-rede
...
barra-tarefas.c      ──exec──>  linux-desktop-down  ──exec──>  transferir-usb
```

Esse diagrama mudou de dono em 07/08/2026: até 06/08 quase tudo saía do
`barra-tarefas.c`. Se um `grep` levar você à barra procurando volume, USB ou
atalho de app, é isto — está no `panorama.c`, no bloco entre `=== a doca ===` e
`=== fim da doca ===`.
...
| o menu da doca é desenhado **dentro** do painel, nunca numa janela com grab próprio | na barra cada menu era uma janela `override_redirect` com `XGrabPointer` seu, e ali isso funcionava porque a barra não tinha grab nenhum. O painel tem o ponteiro **e** o teclado capturados, e o X entrega o ponteiro a **um** cliente só: um segundo grab por cima ou falha, ou rouba os eventos de quem estava embaixo e não os devolve. Desenhar por cima custa um retângulo no mesmo Pixmap de fundo duplo e não disputa nada |
| quem lança **zenity** fecha o painel **antes** do `solta()` | vale para o `Coisas` (o `abrir-windows` pergunta o monitor) e para o `[+]` (o `barra-apps escolher` mostra a lista). Um diálogo aberto por baixo do nosso grab de ponteiro e teclado não recebe clique nem tecla: fica na tela sem responder a nada, e parece que travou. É o mesmo aviso que o comentário do `fechar_janela()` já dava sobre o diálogo de "salvar?" |
| o **lado** do ícone entra no nome do cache do `barra-apps`, junto com a cor | o único teste que o `cmd_listar` faz no cache é `-s`, isto é "não está vazio". Um `.rgb` de 20×20 tem 1200 bytes e passa nesse teste com folga, então seria servido como se fosse de 32×32; o painel leria menos bytes do que espera, recusaria o arquivo e cairia no rótulo de texto — **para sempre**, porque nada jamais o regeneraria. Sem o lado no nome, trocar o tamanho do ícone **apaga** os ícones em vez de aumentá-los |
| a face do botão de app **não** muda com o ponteiro em cima; o realce vai na borda | o ícone chega do `barra-apps` com o alfa já achatado sobre a cor da face. Pintar a face de outro tom no hover põe um quadrado da cor velha visível em volta do ícone — a mesma falha que a invariante do cache de cor existe para evitar, só que disparada pelo mouse |
| o array da doca soma os **três** grupos (`D_FIXOS + MAX_APPS + D_EXTRAS`) | os lançadores entram no meio, em tempo de execução, e o `[+]` entra depois deles. Dimensionado só em `D_FIXOS + MAX_APPS`, encher a doca de atalhos até o teto faz o `[+]` ser escrito uma posição **além do fim**. Não aparece com 2 atalhos fixados, só com 14 — confirmado com controle em 07/08/2026: com 16 no `apps.conf`, o ASan acusa `global-buffer-overflow` no `montar_doca()`, e a versão corrigida passa |
| quem remonta a doca zera o `arrastando` e o `doca_sob` | os dois são **índices** para dentro do array, não ponteiros: nada estoura na hora, eles passam a designar outro item calado. O caminho real é tirar um atalho pelo botão direito, que remonta a doca com um item a menos — e o de índice mais alto era justamente o `[+]`, vizinho do que se removeu |
| o `DOCA_TOTAL` entra nos **dois** lugares do `dispor()` | a altura do cartão e o `y0` de centralização saem da mesma faixa útil. Se o termo faltar num deles não há erro nenhum: os cartões só passam por baixo da doca, meio escondidos, e o sintoma manda procurar no desenho em vez de na conta |
| clique fora de um menu aberto fecha **só o menu**, nunca o painel | o `clique()` do painel fecha tudo quando o clique cai fora dele, e com o ponteiro capturado em `owner_events False` um clique a metros de distância chega aqui do mesmo jeito. Sem a saída antecipada do menu, desistir de escolher um dispositivo de áudio derrubaria o painel junto, e quem só queria cancelar teria de apertar o Win de novo |
| a ociosidade do `soltar-cache` sai do `/proc/stat`, nunca do `/proc/loadavg` | com Xorg, xfwm4 e o Claude de pé a carga fica em 0,2–0,3 **sem ninguém tocar no teclado**: qualquer limiar ali é chute, e errar significa largar o cache no meio do trabalho de quem está usando a sessão. A fração de CPU ocupada por amostragem é a pergunta direta. O `iowait` conta como parado junto com o `idle`, senão sessão esperando disco pareceria ocupada |
| o `soltar-cache` usa `echo 1`, não `echo 3` | o `1` larga o page cache, que é o volume (3907 MB medidos); o `3` levaria junto o dentry/inode, ~50 MB aqui, e a perda deixa todo caminho de arquivo lento de novo sem devolver quase nada |
...
| o watch do inotify **só é trocado quando a pasta muda**, e os eventos são lidos **um a um** | `inotify_rm_watch()` enfileira um `IN_IGNORED` no próprio fd (provado com controle em 06/08/2026: o `rm_watch` sozinho, sem tocar em arquivo nenhum, deixa o fd legível com mask `0x8000`). O laço drena o fd **antes** de recarregar, então trocar o watch a cada recarga deixava o evento nascido do próprio `rm_watch` para a volta seguinte — `select()` acordava na hora, recarregava, tirava o watch outra vez, **para sempre**. Medido: um único `touch` e mais nada, e o binário antigo seguiu consumindo CPU 6 s depois; o corrigido, zero. Drenar às cegas é a outra metade — `IN_IGNORED` não é mudança de pasta |
| o refresh automático da árvore **guarda a rolagem** e recarrega a pasta **da tela**, não a raiz | o `arv_topo = 0` do `carregar_pasta()` existe para *entrar* numa pasta nova; num refresh a pasta é a mesma, e puxar a árvore para o topo a cada arquivo criado é o mesmo erro do `seguir_cursor()` na troca de aba — desfaz a rolagem de quem não pediu nada. E recarregar `projeto` em vez de `pasta` tirava o usuário da subpasta em que ele estava. Quem faz as duas coisas é o `recarregar_arvore()`; ele copia `pasta` para uma local antes, senão o `snprintf` do `carregar_pasta()` seria cópia sobreposta |
...
| quem diz se um USB está presente é o **`BusId`**, nunca a existência do registro no `usbipd state` | o `usbipd bind` grava um registro **persistente** que sobrevive a desligar o aparelho: só o `BusId` cai para `null`. Olhando o `PersistedGuid`, um headset na gaveta responde `windows` igual a um ligado — e como o `resolver_audio()` sai cedo nessa resposta, a detecção casa/trabalho vira código morto em qualquer máquina onde algum headset já tenha sido *bind*-ado. Anexado o `BusId` continua lá, então ele serve aos três estados |
| o `Hw` do `audio-padrao.ps1` sai da chave `,2`, com a `,39` só como primeira tentativa | a `,39` é a documentada e vem **vazia** nos oito endpoints desta máquina. E `'' -like '*VID_…*'` é `False`, então o `Hw` vazio não quebra só o `-ListarUsb`: o `-Evitar` imprime "nada a trocar" sem trocar nada e o `-Restaurar` desiste após ~11 s — os dois **calados**, com saída 0 |
