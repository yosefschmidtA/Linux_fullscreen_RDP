...
/* Qual pasta o inotify_wd esta vigiando. Existe para o watch NAO ser trocado a
 * cada recarga: ver o comentario no fim do carregar_pasta(). */
static char inotify_pasta[PATH_MAX];
...
static XftColor  c_ink, c_fraco, c_sel, c_aberto, c_kw, c_str, c_com;
static XIM       xim;
static XIC       xic;
...
static void carregar_pasta(const char *p)
{
    ...
    /* O WATCH SO E TROCADO QUANDO A PASTA MUDA DE VERDADE, e isto e a metade
     * principal da correcao do laco infinito (06/08/2026).
     *
     * `inotify_rm_watch()` ENFILEIRA um IN_IGNORED no proprio fd. Provado com
     * controle: num programa de dez linhas, o rm_watch sozinho — sem ninguem tocar
     * em arquivo nenhum — deixa o fd legivel, com mask 0x8000.
     *
     * O laco de eventos drena o fd ANTES de recarregar. Trocando o watch a cada
     * recarga, o evento nascido do proprio rm_watch sobrava para a volta seguinte:
     * o select() acordava na hora, recarregava, tirava o watch, gerava outro... e
     * assim para sempre, a 100% de CPU, com a arvore saltando para o topo a cada
     * volta. O gatilho era o PRIMEIRO arquivo criado ou apagado na pasta; dali nao
     * parava mais. */
    if (inotify_fd >= 0 && strcmp(inotify_pasta, pasta) != 0) {
        if (inotify_wd >= 0) inotify_rm_watch(inotify_fd, inotify_wd);
        inotify_wd = inotify_add_watch(inotify_fd, pasta,
                                       IN_CREATE | IN_DELETE | IN_MOVED_TO | IN_MOVED_FROM);
        snprintf(inotify_pasta, sizeof inotify_pasta, "%s", pasta);
    }
}

/* Recarrega a pasta QUE ESTA NA TELA, guardando a rolagem.
 *
 * Nao chame carregar_pasta(pasta): ele copia o argumento para dentro de `pasta`
 * com snprintf, e origem e destino iguais e copia sobreposta. Dai a copia local.
 *
 * A rolagem e devolvida porque o arv_topo = 0 do carregar_pasta() existe para
 * ENTRAR numa pasta nova. Aqui a pasta e a mesma, e puxar a arvore para o topo
 * debaixo de quem esta lendo, a cada arquivo que aparece, e o mesmo erro do
 * seguir_cursor() na troca de aba: desfaz a rolagem de quem nao pediu nada. */
static void recarregar_arvore(void)
{
    char aqui[PATH_MAX];
    int topo_antes = arv_topo;

    snprintf(aqui, sizeof aqui, "%s", pasta);
    carregar_pasta(aqui);
...
static void desenhar_selecao(Drawable buf)
{
    int la, ca, lb, cb, i;
    int vis = linhas_vis();
    ...
    for (i = 0; i < vis && topo + i < n_linhas; i++) {
        ...
        XFillRectangle(dpy, buf, gc, x0, y,
...
static void desenhar_rolagem(Drawable buf)
{
    int h = cont_h(), x = cont_w() - ROL_W, ty, th;

    XSetForeground(dpy, gc, FACE_ESC);          /* o trilho */
    XFillRectangle(dpy, buf, gc, x, 0, ROL_W, (unsigned) h);
    linha_px(buf, x, 0, x, h - 1, SH);

    if (rolagem_cursor(&ty, &th))
        bisel(buf, x + 1, ty, ROL_W - 1, th, (int) FACE, 0);
}
...
static void desenhar_numeros(Drawable buf)
{
    int gw = numeros_w(), h = cont_h(), vis = linhas_vis(), i;

    if (gw <= 0) return;
    XSetForeground(dpy, gc, FACE);
    XFillRectangle(dpy, buf, gc, 0, 0, (unsigned) gw, (unsigned) h);
    linha_px(buf, gw - 1, 0, gw - 1, h - 1, SH);
...
static void desenhar_editor(void)
{
    ...
    if (w <= 0 || h <= 0) return;
    buf = XCreatePixmap(dpy, w_ed, (unsigned) w, (unsigned) h, DefaultDepth(dpy, tela));
    XftDrawChange(dr_ed, buf);
    ...
    if (cur_l >= topo && cur_l < topo + vis) {
        ...
        if (cx >= texto_x0() - 1) {
            XSetForeground(dpy, gc, INK);
            XFillRectangle(dpy, buf, gc, cx, cy, 2, (unsigned) altura_lin);
        }
    }

    desenhar_numeros(buf);
    desenhar_rolagem(buf);

    XCopyArea(dpy, buf, w_ed, gc, 0, 0, (unsigned) w, (unsigned) h, 0, 0);
    XftDrawChange(dr_ed, w_ed);
    XFreePixmap(dpy, buf);
}
...
static void tecla(XKeyEvent *ev)
{
    ...
    /* F5 recarrega a arvore de arquivos (util quando scripts criam arquivos) */
    if (ks == XK_F5) {
        carregar_pasta(projeto);
        desenhar();
        return;
...
#undef XCOR
    ...
    for (;;) {
        if (!XPending(dpy)) {
            ...
            select((x_fd > inotify_fd ? x_fd : inotify_fd) + 1, &fds, NULL, NULL, NULL);
            ...
            if (inotify_fd >= 0 && FD_ISSET(inotify_fd, &fds)) {
                ...
                /* OS EVENTOS SAO LIDOS UM A UM, e nao drenados as cegas. Tratar
                 * tudo o que chega como "a pasta mudou" fazia o IN_IGNORED do
                 * proprio rm_watch valer como mudanca — a outra metade do laco
                 * infinito. Aqui so os quatro eventos pedidos contam. */
                while ((n = read(inotify_fd, buf, sizeof buf)) > 0) {
                    char *p;
                    for (p = buf; p < buf + n; ) {
                        struct inotify_event *e = (struct inotify_event *) p;
                        if (e->mask & (IN_CREATE | IN_DELETE | IN_MOVED_TO | IN_MOVED_FROM))
                            mudou = 1;
                        p += sizeof *e + e->len;
                    }
                }

                /* recarregar_arvore(), e nao carregar_pasta(projeto): a arvore pode
                 * estar dentro de uma subpasta, e recarregar a RAIZ tirava o
                 * usuario de onde ele estava a cada arquivo criado. */
                if (mudou) {
                    recarregar_arvore();
                    desenhar();
                }
